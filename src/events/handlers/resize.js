  ...
  if (current.isVisible && current.active) {
    setTimeout(function() {
      var maxInputPercentage = 35,
        ...
        cloneHeight = (current.input) ? _cloneHeight(current.inputClone) : null,
      ...
      setTimeout(function() {
        if (current.input)
          inputHolderHeight = Math.floor(current.inputHolder.offsetHeight);
        chatHolderHeight = Math.floor(current.chatHolder.offsetHeight);
