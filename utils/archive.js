...
var docssrc = path.resolve( __dirname, '../docs/JSDOCS.md');
var docsdest = path.resolve( __dirname, '../dist', `IBMChatClient-v${pkg.version}-jsdocs.md`);
var readmesrc = path.resolve( __dirname, '../docs/DOCS.md');
var readmedest = path.resolve( __dirname, '../dist', `IBMChatClient-v${pkg.version}.md`);
...
fs.copy(chatsrc, chatdest, function(err) {
	...
	fs.copy(docssrc, docsdest, function(err) {
		if (err) return console.error(`Failed copying ${docsdest}`, err);
		console.log(`Copied ${docsdest}`);
		fs.copy(quickstartsrc, quickstartdest, function(err) {
			...
			fs.copy(readmesrc, readmedest, function(err) {
				if (err) return console.error(`Failed copying ${readmedest}`, err);
				console.log(`Copied ${readmedest}`);
				glob(`${dest}/*-jsdocs.md`, function(er, files) {
					...
					for (var i = 0; i < files.length; i++) {
						if (files[i].indexOf('versions') === -1) {
							...
							json.versions.push({
								version: version,
								apidocs: 'IBMChatClient-v' + version + '-jsdocs.md',
								docs: 'IBMChatClient-v' + version + '.md',
						...
						if (i === (files.length - 1)) {
							...
							setTimeout(function() {
								fs.writeFile(`${versionssrc}`, beautify(JSON.stringify(json), { indent_size: 2 }), function(err) {
